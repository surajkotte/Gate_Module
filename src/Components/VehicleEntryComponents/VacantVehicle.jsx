...
const VacantVehicle = ({ onSaveClick, onSubmitClick, displayOnly = false }) => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      const config = await getConfig("vacant_vehicle");
      if (config.messageType === "S") {
        console.log("Fetched vacant_vehicle config:", config);
        setFormData({
        ...
        console.error("Failed to fetch vacant_vehicle config");
      }
    };
    fetchConfig();
  }, []);
    ...
    <div className="w-full flex flex-col gap-5">
        ...
        <div className="flex gap-3">
          {!displayOnly && (
            <>
              <Button
                variant="secondary"
                onClick={() => onSaveClick(formData, "vacant_vehicle")}
                className="gap-2"
              >
              ...
              <Button
                className="gap-2"
                variant="default"
                onClick={() => onSubmitClick(formData, "vacant_vehicle")}
              >
      ...
      <div className="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-400">
        <p className="text-blue-700 text-sm">
          For Vacant Vehicle entries, please ensure all mandatory fields are
